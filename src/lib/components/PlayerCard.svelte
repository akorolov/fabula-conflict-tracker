...
<div class="card bg-base-100 rounded-lg {hasActed ? 'border-3 border-primary bg-base-200' : 'border border-base-content/10'}">
			...
			{#if activeStatuses.length > 0}
				<div class="flex gap-1 flex-wrap mt-2">
					{#each activeStatuses as status (status.key)}
						<span class="badge badge-secondary badge-sm">{status.label}</span>
					{/each}
				</div>
			{/if}
		</div>
